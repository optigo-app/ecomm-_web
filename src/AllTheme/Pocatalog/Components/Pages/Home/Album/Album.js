    ...
    <div className="proCat_alubmMainDiv">
      ...
      <div className="proCat_albumALL_div" style={{ minHeight: !albumData.length && '600px' }}>
        {albumData.map((data, index) => {
          let imgSrc = imageNotFound; // Default to imageNotFound
              ...
              if (detail?.Designdetail) {
                ...
                designDetails.some((design) => {
                  if (design.ImageCount > 0) {
                    ...
                    return true; // Stop searching after finding the first valid image
                  }
                  return false;
                });
              }
...
//   const fetchAndSetAlbumData = async (value, finalID) => {
//     const storeInit = JSON.parse(sessionStorage.getItem("storeInit"));
//     let RegNo = storeInit?.FrontEnd_RegNo || (htmlContent?.rd && htmlContent.rd[0]?.FrontEnd_RegNo);
//     let DomainNo = storeInit?.DomainForNo || (htmlContent?.rd && htmlContent.rd[0]?.DomainForNo);
//     console.log('fetchAndSetAlbumData RegNo', RegNo);
//     console.log('fetchAndSetAlbumData DomainNo', DomainNo);
//     if (!RegNo) {
//       return;
//     }
//     if (!DomainNo) {
...
//     }
//     try {
//       const response = await Get_Procatalog("GETProcatalog", finalID, value, RegNo, DomainNo);
//       if (response?.Data?.rd) {
//         const albums = response.Data.rd;
//         setAlbumData(albums);
