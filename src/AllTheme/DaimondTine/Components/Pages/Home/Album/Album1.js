...
const Album1 = () => {
  ...
  const handleNavigate = (album) => {
    navigation(
      `/p/${album?.AlbumName}/?A=${btoa(`AlbumName=${album?.AlbumName}`)}`
    );
  };
  ...
  return (
    <div ref={albumRef} aria-label="Album-container" >
        ...
        <div className="dt_album_container">
          ...
          >
            <Tabs
              ...
              variant="scrollable"
              scrollButtons="auto"
              aria-labelledby="album-title" // Link the tabs to the album title for better context
              aria-label="Album selection tabs"
              TabIndicatorProps={{
                style: { display: "none" },
              }}
            >
              {albumData?.map((album) => (
                <Tab
                  ...
                  aria-label={`Select ${album?.AlbumName} album`}
            ...
            {albumData?.map((album) =>
              album?.AlbumName === selectedAlbum ? (
                <Swiper
                  ...
                  aria-label={`Swiper for ${album?.AlbumName}`} // Provide context for the swiper component
                >
                  {album?.Designdetail &&
